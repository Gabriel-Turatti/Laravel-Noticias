...
<div class="container">
    <div class="row mt-3">
        <div class="col-12">
            ...
            {{ method_field('PUT') }}
            @else
            <form action="{{ route('options.store', ['id' => $option->id, 'question_id' => $option->question_id]) }}" method="POST">
            @endif

                {{ csrf_field() }}
                <input type="hidden" name="question_id" id="question_id" value="{{ $option->question_id }}"></input>
                <div class="form-row">
                ...
                <div class="form-row">
                    <div class="form-group col-md-12">
                        <div class="custom-control custom-switch">
                            @if($option->correct)
                                <input class="custom-control-input" type="checkbox" name="correct" id="correct" value="S" checked>
                            @else
                                <input class="custom-control-input" type="checkbox" name="correct" id="correct" value="S">
                            @endif
